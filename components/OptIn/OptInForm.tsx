  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedNumber = phoneNumber.startsWith('+') 
        ? phoneNumber 
        : `+234${phoneNumber.startsWith('0') ? phoneNumber.substring(1) : phoneNumber}`;
      ...
      setError(error.response?.data?.message || 'Failed to opt-in. Please try again.');
      setFailure(true);
    } finally {
      ...
      {!success && !failure ? (
        <>
          <p style={{ marginBottom: '1.5rem' }}>
            Enter your MTN phone number to participate in the MyNumba Don Win promotion.
            You'll be eligible for daily and weekly draws with chances to win amazing prizes!
          </p>
        ...
        <SuccessAnimation>
          ...
          <p style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            Your phone number has been registered for the MTN MyNumba Don Win promotion.
            You are now eligible for daily and weekly draws!
            {email && (
              <><br /><br />A confirmation email has been sent to <strong>{email}</strong>.</>
            )}
          </p>

...
const TemplateList: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setMessage({ type: 'danger', text: 'Failed to delete template. Please try again.' });
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <Card>
      ...
      {loading ? (
        ...
        <NoTemplatesMessage>
          <FaEnvelope size={48} style={{ opacity: 0.3, marginBottom: '1rem' }} />
            ...
            <tbody>
              {templates.map((template) => (
                <tr key={template.id}>
                  ...
                  <td>
                    <Badge variant={template.type === 'winner' ? 'success' : 'primary'}>
                      {template.type}
                    </Badge>
                  </td>

...
import { Table, Badge } from "@/src/components/common/styles"
        ...
        <tbody>
          {winners.length > 0 ? (
            winners.map(winner => (
              <tr key={winner.id}>
                <td>
                  <PhoneNumber>{formatMSISDN(winner.msisdn)}</PhoneNumber>
                </td>
                <td>
                  <Amount>{formatAmount(winner.amount)}</Amount>
                </td>
                <td>{formatDate(winner.drawDate)}</td>
                <td>
                  <Badge
                    variant={
                      winner.drawType === "weekly" ? "secondary" : "primary"
                    }
                  >

export const joinSteps = [
  {
    icon: "/images/step1.png",
    title: "Step 1:",
    desc: "Opt-In to be eligible to participate and win a prize.",
  },
  {
    icon: "/images/step2.png",
    title: "Step 2:",
    desc: "Recharge your MTN line, understand how points work and how to maximise your chances <a className='text-yellow-400/80' href='/prizes#how-we-select-winners'>here</a>.",
  },
  {
    icon: "/images/step3.png",
    title: "Step 3:",
    desc: "Keep up with draw results and claim your prizes when you win.",
  },
];

export const whyJoin = [
  {
    title: "Benefit 1:",
    label: "Daily Chances to Win",
    desc: "Every day brings a new opportunity to become a millionaire!",
    bg: "bg-yellow-400",
    image: "/images/benefit-1.png",
  },
  {
    title: "Benefit 2:",
    label: "Simple Point System",
    desc: "The more you recharge, the more chances you have to win. The points reflect how many times your number is entered into the draw",
    bg: "bg-[#0056B3]",
    image: "/images/benefit-2.png",
  },
  {
    title: "Benefit 3:",
    label: "Instant Notifications",
    desc: "Know immediately when you’ve won!",
    bg: "bg-[#0056B3]",
    image: "/images/benefit-3.png",
  },
  {
    title: "Benefit 4:",
    label: "Easy Prize Collection",
    desc: "Winnings sent directly to your account!",
    bg: "bg-yellow-400",
    image: "/images/benefit-4.png",
  },
];

...
export const authAPI = {
  login: async (credentials: { phoneNumber: string; token: string }) => {
    const response = await api.post("/auth/login", credentials)
    return response.data
  },
  requestLoginToken: async (credentials: { phoneNumber: string }) => {
    const response = await api.post("/auth/request-login-token", credentials)
    return response.data
  },
  logout: () => {
    Cookies.remove("token")
...
export const topupAPI = {
  getTopups: async (
    ...
    const response = await api.get(
      `/topups?startDate=${startDate}&endDate=${endDate}&page=${page}&limit=${limit}`
    )
    return response.data
  },
  ...
  getTopupsByMSISDN: async (msisdn: string, page = 1, limit = 10) => {
    const response = await api.get(
      `/topups/msisdn/${msisdn}?page=${page}&limit=${limit}`
    )
    return response.data
  },
...
export const drawAPI = {
  getDraws: async (
    ...
    const response = await api.get(
      `/draws?startDate=${startDate}&endDate=${endDate}&page=${page}&limit=${limit}`
    )
    return response.data
  },
  getDrawHistory: async (drawId: string) => {
    const response = await api.get(`/draws/${drawId}/history`)
    return response.data
  },
  ...
  getDrawsByStatus: async (status: string, page = 1, limit = 10) => {
    const response = await api.get(
      `/draws/status/${status}?page=${page}&limit=${limit}`
    )
    return response.data
  },
...
export const notificationAPI = {
  ...
  getNotificationsByMSISDN: async (msisdn: string, page = 1, limit = 10) => {
    const response = await api.get(
      `/notifications/msisdn/${msisdn}?page=${page}&limit=${limit}`
    )
    return response.data
  },
  getNotificationsByCampaignID: async (
    campaignID: string,
    page = 1,
    limit = 10
  ) => {
    const response = await api.get(
      `/notifications/campaign/${campaignID}?page=${page}&limit=${limit}`
    )
    return response.data
  },
  ...
  getCampaigns: async (page = 1, limit = 10) => {
    const response = await api.get(
      `/notifications/campaigns?page=${page}&limit=${limit}`
    )
    return response.data
  },
  ...
  getTemplates: async (page = 1, limit = 10) => {
    const response = await api.get(
      `/notifications/templates?page=${page}&limit=${limit}`
    )
    return response.data
  },
  ...
  getTemplatesByType: async (type: string, page = 1, limit = 10) => {
    const response = await api.get(
      `/notifications/templates/type/${type}?page=${page}&limit=${limit}`
    )
    return response.data
  },
